...
const shippingRoutes = require("./routes/shippment")
...
app.post('/create-product', async (req, res) => {
  try {
    ...
    const files = req.files?.images; // Assume `images` is an array of files
    ...
    // If only one file is uploaded, `req.files.images` might not be an array.
    ...
    if (!seller || seller._id.toString() !== req.session.sellerId) {
    ...
    const product = new Product({
      ...
      productId: uuidv4(),
      inStockValue: productData.inStockValue || 0,
      soldStockValue: productData.soldStockValue || 0,
      images: uploadedImages,
      visibility: productData.visibility || 'on',
...
app.delete('/remove-product/:productId', async (req, res) => {
  try {
    ...
    if (product.sellerId && product.sellerId.toString() !== req.session.sellerId) {
      return res.status(403).json({
        success: false,
