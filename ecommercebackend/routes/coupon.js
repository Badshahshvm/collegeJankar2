...
const router = express.Router();
const Coupon = require('../models/couponModel');
const User = require('../models/userModel'); // Adjust path as per your User model
...
router.post('/save-coupon', async (req, res) => {
  try {
    ...
    // Notify users
    const subject = 'New Coupon Available!';
    const message = `A new coupon "${code}" with ${discountPercentage}% discount is now available. Use it before ${expiresAt}!`;
    await sendEmailToAllUsers(subject, message);

    res.status(201).json({ success: true, message: 'Coupon created successfully', coupon });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error saving coupon', error: error.message });
  }
});
